...
int main(int argc, char* argv[])
{
		...
		MODE_FILTER_OPT(MODE::IM)
		{
			...
			p1.run<im_outputs_t>(
					[&](const WHEN when, const ind step, const uind pass, auto& wf)
					{
						...
						if(when == WHEN::AT_END) wf.save(im_output_name);
					});
		}
	...
	if constexpr
		MODE_FILTER_OPT(MODE::RE)
		{
			...
			using re_outputs_t= BufferedBinaryOutputs<VALUE<Step, Time>,
																								VALUE<F1>,
																								AVG<Identity>,
																								AVG<PotentialEnergy>
			...
			p2.run<re_outputs_t>(
					[=](const WHEN when, const ind step, const uind pass, auto& wf)
					{
						if(when == WHEN::AT_START) wf.load(im_output_name);

						if(when == WHEN::DURING && step % 50 == 0) wf.snapshot("step_" + std::to_string(step));
