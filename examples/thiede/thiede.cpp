...
cxxopts::ParseResult getOpts(const int argc, char* argv[])
{
	options.add_options()("h,help", "Print help")("n,nodes",
			...
			"x,dx", "Set the spacedelta [a.u.]", cxxopts::value<double>()->default_value("0.2"))(
			"t,dt", "Set the timedelta [a.u.]", cxxopts::value<double>()->default_value("0.2"))(
			"s,soft",
...
int main(const int argc, char* argv[])
{
	...
	{
		if(!MPI::pID) std::cout << options.help({"", "Environment", "Testing", "Laser"}) << std::endl;
		QSF::finalize();
		exit(0);
	}
	...
	if constexpr
		...
		{
			QSF::subdirectory("n" + std::to_string(nodes) + "_" + std::to_string(dx) + "/F0_" +
												std::to_string(field) + "/omega_" + std::to_string(omega));
			...
			DipoleCoupling<VelocityGauge, A1> re_coupling{A1{
					Sin2Envelope<SinPulse>{{.field						= field * sqrt(3.) * 0.5 / omega,
																	.omega						= omega,
																	.ncycles					= ncycles,
																	.phase_in_pi_units= phase_in_pi_units,
																	.delay_in_cycles	= delay_in_cycles}},
					ConstantPulse{{.field= 0.0, .omega= omega, .ncycles= 3.0, .delay_in_cycles= ncycles}}}};
