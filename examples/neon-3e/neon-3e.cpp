...
int main(const int argc, char* argv[])
{
	...
	ReducedDimInteraction<ReducedModel::EckhardSacha> potential{
			{.Ncharge= 3.0 / sqrt(1.0), .Echarge= -sqrt(1.0), .Nsoft= 0.83, .Esoft= 0.83}};
	...
	if constexpr
		MODE_FILTER_OPT(MODE::IM)
		{
			...
			auto p1= SplitPropagator<MODE::IM, SplitType, decltype(im_wf)>{
					PropagatorBase{.dt= 0.3, .max_steps= 1000000, .state_accuracy= 10E-15}, std::move(im_wf)};
	...
	if constexpr
		MODE_FILTER_OPT(MODE::RE)
		{
			...
			IO::path im_output=
					IO::root_dir / IO::path("groundstates/" + std::to_string(nodes) + "_repX");
